...
class RcpTableListViewTest(TestCase):
    ...
    def test_retrieve_table_list(self):
        ...
        self.assertEqual(len(response.context['table_list']), 12)
...
class RcpNmListViewTest(TestCase):
    ...
    def test_retrieve_table_list(self):
        ...
        self.assertTrue('is_paginated' in response.context)
        self.assertTrue(response.context['is_paginated'] == True)
        self.assertEqual(len(response.context['rcp_nm_list']), 12)
        last_rcp_num = 15*5
        for i in range(last_rcp_num, last_rcp_num-12, -1):
...
class RcpCreateViewTest(TestCase):
    ...
    def test_create_rcp(self):
        ...
        self.assertEqual(response.status_code, 302)
        new_rcp = RcpTable.objects.get(rcp_num=(self.rcp.rcp_num + 1))
